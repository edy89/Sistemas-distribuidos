...
class ClientThread(threading.Thread):
    ...
    def __init__(self):
      ...
      self.client  = xmlrpclib.ServerProxy('http://localhost:8558',allow_none = True)
      self.id      = 2
      self.page1   = page('page1.txt',1)
      self.page2   = page('page2.txt',2)
      self.page1.write_('Page1 : PC2 \n')
      self.page2.write_('Page2 : PC2 \n')
      self.register   = open ('register.txt','w')
      self.register.write('register: ')
      self.register.close()
      self.client.catch_page('C:/Users/Edyson/Desktop/Sistemas distribuidos/Final/PC2/page1.txt',self.id,self.page1.id)
      self.client.catch_page('C:/Users/Edyson/Desktop/Sistemas distribuidos/Final/PC2/page2.txt',self.id,self.page2.id)
             ...
             if pc == '1' or pc == '2' or pc == '3':
               ...
               if  int(opage) <= len(pages_pc):
                 if not self.client.is_used(int(pc),int(opage)):
                   ...
                   self.client.register_in(int(pc),int(opage),'http://localhost:8558')
                 else:
                   print 'The page you need is not available rigth now, wait for it...'
                   temp = self.client.put_in_queue('http://localhost:8558',int(pc),int(opage))
                   self.client.add_page_p(int(pc),int(opage))
                   file = open(pages_pc[int(opage) - 1],'r+')
                   content = file.readlines()
                   #file.write('page' + opage + ':PC2')
